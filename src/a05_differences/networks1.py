...
class Correlation(nn.Module):
	...
	def operation(a, b):
		...
		# print(torch.max(cor), torch.min(cor), torch.mean(cor))
		# print("================================")
		# input()
...
class ComparatorImageToGenAndLabels(nn.Module):
	...
	class CatMixCorrWithSem(nn.Module):
		...
		def forward(self, prev, feats_img, feats_rec, feats_sem):
			...
			# print(torch.max(feats_img), torch.min(feats_img), torch.mean(feats_img))
			# print(torch.max(feats_rec), torch.min(feats_rec), torch.mean(feats_rec))
			# print(torch.max(feats_sem), torch.min(feats_sem), torch.mean(feats_sem))
